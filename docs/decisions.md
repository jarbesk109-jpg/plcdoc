...
- **Ownership:** `Pou`, `GlobalVarList`, `Task` and `Variable` retain the enclosing configuration and application (the resource name). `None` means project/configuration scope, not an inferred application. Variable identity is `(application, scope, name)`; projects with multiple configurations group by configuration first. Deduplication compares POU definitions only within `(configuration, application, name)`. Identical copies collapse with a warning; conflicting definitions remain with a warning. Project POUs are independent of resource-local POUs.
...
- **Declarations:** Recursive arrays and sized STRING/WSTRING types have readable names; `derived_types` exposes derived bases even inside arrays. Canonical `type_xml`/`initial_value_xml` also distinguish declarations whose readable forms coincide. Array/struct initial values and unsupported types use canonical XML as their displayed value/type. The model contains strings and dataclasses, never live XML elements.
...
- **Normalization:** Canonical XML normalizes namespace prefixes, attribute order and element-only indentation while preserving leaf/mixed text and `xml:space`. Existing documented exclusions remain: GUIDs, timestamps, device/library metadata, graphical positions, LD network comments and CODESYS network titles. Unknown vendor elements are preserved, even when their local names resemble layout tags. ST text retains whitespace apart from normalized line endings. M4 must choose semantic fields rather than compare project headers or diagnostics.
