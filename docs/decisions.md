...
- **Normalization:** Canonical XML is C14N 2.0 with `rewrite_prefixes=True` and `strip_text=True`: prefixes become `n0`, `n1`, ... regardless of the process-wide `ElementTree.register_namespace` state, attribute order is sorted and formatting whitespace is stripped. Existing documented exclusions remain: GUIDs, timestamps, device/library metadata, graphical positions, LD network comments and CODESYS network titles. Unknown vendor elements are preserved, even when their local names resemble layout tags. ST text retains whitespace apart from normalized line endings. M4 must choose semantic fields rather than compare project headers or diagnostics.
